...
//! Обработчики для экспериментов (v2.0)
...
pub async fn create_experiment(
    ...
    sqlx::query(r#"
        ...
        (id, title, description, experiment_date, experiment_type, instructor, student_group, location, status, created_by, updated_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, ?)
    "#)
        .bind(&id)
...
pub async fn get_experiment_reagents(
    ...
    let reagents: Vec<ExperimentReagentWithDetails> = sqlx::query_as(r#"
        SELECT 
            er.id, er.experiment_id, er.batch_id, er.quantity_planned, 
...
pub async fn add_reagent_to_experiment(
    ...
    if body.quantity_planned > available {
        return Err(ApiError::insufficient_quantity(available, body.quantity_planned));
    }
    ...
    sqlx::query(r#"
        INSERT INTO experiment_reagents (id, experiment_id, batch_id, quantity_planned, notes, created_at)
        VALUES (?, ?, ?, ?, ?, ?)
    "#)
        .bind(&id)
        .bind(&experiment_id)
        .bind(&body.batch_id)
        .bind(body.quantity_planned)
        .bind(&body.notes)
        .bind(&now)
        .execute(&mut *tx)
        .await?;
    ...
    let created: ExperimentReagent = sqlx::query_as("SELECT * FROM experiment_reagents WHERE id = ?")
        .bind(&id)
        .fetch_one(&app_state.db_pool)
        .await?;
...
pub async fn remove_reagent_from_experiment(
    ...
    // Get the link to know how much to unreserve
    let link: ExperimentReagent = sqlx::query_as(
